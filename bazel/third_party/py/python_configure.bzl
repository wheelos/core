...
load("//:bazel/common.bzl", "basename")
...
def _tpl(repository_ctx, tpl, substitutions = {}, out = None):
    ...
    repository_ctx.template(
        out,
        Label("//bazel/third_party/py:%s.tpl" % tpl),
        substitutions,
    )
...
python_configure = repository_rule(
    ...
    attrs = {
        "_build_tpl": attr.label(
            default = Label("//bazel/third_party/py:BUILD.tpl"),
            allow_single_file = True,
        ),
        "_variety_tpl": attr.label(
            default = Label("//bazel/third_party/py:variety.tpl"),
            allow_single_file = True,
        ),
    },
)
